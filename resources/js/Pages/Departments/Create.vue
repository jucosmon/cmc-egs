                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex justify-between items-center mb-6">
                            <h2 class="text-2xl font-bold text-gray-800">Create Department</h2>
                        ...
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <!-- Dean -->
                            <div>
                                <label for="dean_id" class="block text-sm font-medium text-gray-700">
                                    Dean (Optional)
                                </label>
                                <select
                                    id="dean_id"
                                    ...
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                >
                                    <option :value="null">Select a dean...</option>
                                    <option v-for="dean in deans" :key="dean.id" :value="dean.id">
                                        {{ dean.name }} ({{ dean.official_id }})
                                    </option>
                                </select>
                                <div v-if="form.errors.dean_id" class="text-red-600 text-sm mt-1">
                                    {{ form.errors.dean_id }}
                                </div>
                            </div>
